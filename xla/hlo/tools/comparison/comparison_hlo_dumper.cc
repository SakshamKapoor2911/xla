...
std::string GetTooltipData(const HloNodeComparisonStats* comp,
  ...
  if (comp != nullptr) {
    ...
    if (!comp->not_comparable) {
      json["diffScore"]["count"] = comp->score_count;
  ...
  auto add_metric = [&](absl::string_view label, auto get_val) {
    if (baseline != nullptr) {
      json["metrics"][label]["baseline"] = (double)get_val(*baseline);
    }
    if (target != nullptr) {
      json["metrics"][label]["target"] = (double)get_val(*target);
    }
  };
...
void GenerateHloHtmlDumps(int replica_id, const HloModule& baseline_module,
  ...
  riegeli::RecordReader reader{riegeli::FileReader(comparison_results_path)};
...
void GenerateSingleHloHtmlDump(int replica_id, const HloModule& module,
  ...
  riegeli::RecordReader reader{riegeli::FileReader(recovered_summaries_path)};
