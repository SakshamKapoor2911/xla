...
class MockComparisonTool : public ComparisonTool {
 public:
  MockComparisonTool(const ComparisonOptions& options,
...
TEST_F(ComparisonToolTest, CreateTensorSummaryEmptyLiteral) {
  ...
  mock_tool_ =
      std::make_unique<MockComparisonTool>(options, thread::DefaultQueue());
...
TEST_F(ComparisonToolTest, CreateTensorSummaryAllZeros) {
  ...
  EXPECT_THAT(summary.checksum(), StrEq("\x9B*8\x18\x87k[\x94"));
...
TEST_F(ComparisonToolTest, CreateTensorSummarySimpleFloatsAndSampling) {
  ...
  EXPECT_THAT(summary.checksum(), StrEq("\x8A\xB9u\xCC\xBF:\x84\xCC"));
...
TEST_F(ComparisonToolTest, CreateTensorSummaryWithScopes) {
  mock_tool_ = std::make_unique<MockComparisonTool>(ComparisonOptions(),
                                                    thread::DefaultQueue());
...
TEST_F(ComparisonToolTest, RecordTensorHloModuleNoMatch) {
  ...
  thread::DefaultQueue()->WaitUntilComplete();
}
