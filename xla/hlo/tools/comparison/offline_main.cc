...
#include "third_party/gloop/util/random/global_id.h"
#include "re2/re2.h"
...
void PrintComparisonSummary(int replica_id,
  ...
  riegeli::RecordReader reader(
      riegeli::Maker<riegeli::FileReader>(comparison_results_path));
...
class RiegeliShardReader
    : public riegeli::JoiningReader<riegeli::FileReader<>> {
 public:
 ...
 protected:
  bool OpenShardImpl() override {
    ...
    shard() = riegeli::FileReader(filepath);
    if (!shard().ok()) {
...
absl::StatusOr<RecoveryRunResults> RecoverJob(
  ...
  for (int i = 0; i < replica_count; ++i) {
    writers[i] = riegeli::Maker<riegeli::RecordWriter>(
        riegeli::Maker<riegeli::FileWriter>(output_filenames[i]),
  ...
  auto [recoverer, creation_metrics] = *std::move(recoverer_or);
...
void Run() {
  ...
  std::string temp_file_dir = tsl::io::JoinPath(
      temp_file_base_dir, absl::StrCat(util::random::NewGlobalID()));
  ...
  if (single_run_mode) {
    ...
    if (comparison_files_exist) {
      ...
      for (int i = 0; i < replica_count; ++i) {
        writers[i] = riegeli::Maker<riegeli::RecordWriter>(
            riegeli::Maker<riegeli::FileWriter>(comparison_output_filenames[i]),
            riegeli::RecordWriterBase::Options().set_transpose(true));
      }
      absl::Mutex writer_mutex;
      ...
      for (int i = 0; i < replica_count; ++i) {
        ...
        riegeli::RecordReader baseline_reader(
            riegeli::Maker<riegeli::FileReader>(
                baseline_results.output_filenames[i]));
        riegeli::RecordReader target_reader(riegeli::Maker<riegeli::FileReader>(
            target_results.output_filenames[i]));
