...
FloatSummary ApplyReshapeToSummary(
  ...
  return {.block_summaries = new_block_summaries, .split_spec = new_split_spec};
}
...
AbsoluteScopedTensorKey GetAbsoluteScopedTensorKey(
  ...
  for (const auto& scope : log_hlo_output_metadata.scopes()) {
    scope_instructions.push_back(ScopeInstruction::Create(
        google::protobuf::StringCopy(scope.instruction_name()), scope.it_count()));
  }
  return AbsoluteScopedTensorKey::Create(
      TensorKey::Create(
          google::protobuf::StringCopy(log_hlo_output_metadata.instruction_name()),
