...
#include "third_party/gloop/util/gtl/extend/ordering.h"
...
struct ScopeInstruction
    : gtl::Extend<ScopeInstruction, 2>::With<gtl::OrderingExtension,
  ...
  static ScopeInstruction Create(::absl::string_view instruction_name,
                                 int64_t iteration_index = 0) {
    return {.instruction_name = std::string(instruction_name),
            .iteration_index = iteration_index};
  }

  static ScopeInstruction FromString(::absl::string_view instruction_name) {
    if (instruction_name.ends_with("#*")) {
...
struct TensorKey
    : gtl::Extend<TensorKey, 2>::With<gtl::OrderingExtension,
                                      gtl::DebugPrintingExtension> {
...
struct ScopedTensorKey
    : gtl::Extend<ScopedTensorKey, 2>::With<gtl::OrderingExtension,
                                            gtl::DebugPrintingExtension> {
  ...
  static ScopedTensorKey Create(
      TensorKey tensor_key,
      std::vector<ScopeInstruction> scope_instructions = {}) {
    return {.scope_instructions = std::move(scope_instructions),
