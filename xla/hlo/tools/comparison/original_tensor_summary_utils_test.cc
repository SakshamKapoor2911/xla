...
TEST(ApplyNonUnshardTensorTransformationToSummaryTest, SingleReshape) {
  ...
  original_summary.summaries[0].split_spec = {
      {.dim_index = 0, .block_count = 2}, {.dim_index = 1, .block_count = 2}};
...
TEST(TensorTransformationTest, Equality) {
  auto reshape1 = Reshape{.continuation = nullptr, .output_dimensions = {2, 2}};
  auto reshape2 = Reshape{.continuation = nullptr, .output_dimensions = {2, 2}};
  auto reshape3 = Reshape{.continuation = nullptr, .output_dimensions = {2, 3}};
...
TEST(ScopeInstructionTest, ToString) {
  ScopeInstruction scope1 = {.instruction_name = "loop", .iteration_index = 3};
  EXPECT_EQ(scope1.ToString(), "loop#3");
  ScopeInstruction scope2 = {.instruction_name = "call", .iteration_index = 0};
  EXPECT_EQ(scope2.ToString(), "call");
  ScopeInstruction scope3 = {.instruction_name = "wild", .iteration_index = -1};
...
TEST(ScopedTensorKeyTest, FromString) {
  EXPECT_EQ(ScopedTensorKey::FromString("loop#3/hlo", {0, 1}),
            (ScopedTensorKey{.scope_instructions = {{.instruction_name = "loop",
                                                     .iteration_index = 3}},
                             .tensor_key = {.instruction_name = "hlo",
                                            .shape_index = {0, 1}}}));

  EXPECT_EQ(ScopedTensorKey::FromString("call/hlo"),
            (ScopedTensorKey{
                .scope_instructions = {{.instruction_name = "call",
                                        .iteration_index = 0}},
                .tensor_key = {.instruction_name = "hlo", .shape_index = {}}}));
  ...
  EXPECT_EQ(ScopedTensorKey::FromString("outer#1/inner#2/hlo"),
            (ScopedTensorKey{
                .scope_instructions =
...
TEST(GetAbsoluteScopedTensorKeyTest, Conversion) {
  ...
  AbsoluteScopedTensorKey expected_key = {
      .scope_instructions = {{.instruction_name = "loop", .iteration_index = 3},
